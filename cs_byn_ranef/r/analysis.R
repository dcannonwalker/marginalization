...
G <- sim_list$G
# stan_post <- readRDS("cs_byn_ranef/data/stan_fit.rds")
# stan_smry <- stan_post$summary()
# stan_smry <- readRDS("cs_byn_ranef/data/stan_smry.rds")

nb_post <- readRDS("cs_byn_ranef/data/nb_fit.rds")
# nb_smry <- nb_post$summary()
nb_smry <- readRDS("cs_byn_ranef/data/nb_smry.rds")

eR_fit <- readRDS("cs_byn_ranef/data/erfit.rds")
...
eR_b <- tibble::tibble(estimate = eR_fit$tt$table$logFC, variable = nb_b$variable, model = "edger")
...
saveRDS(b1_plot, file = "cs_byn_ranef/data/b1_plot.rds")
# saveRDS(b0_plot, file = "cs_byn_ranef/data/b0_plot.rds")
# saveRDS(S_plot, file = "cs_byn_ranef/data/S_plot.rds")
saveRDS(roc_plot, file = "cs_byn_ranef/data/roc_plot.rds")
saveRDS(fdr_plot, file = "cs_byn_ranef/data/fdr_plot.rds")
saveRDS(phi_plot, file = "cs_byn_ranef/data/phi_plot.rds")

nb_u <- nb_smry %>%
  filter(grepl("^u\\[", variable)) %>%
  select(variable, mean, sd) %>%
  mutate(model = "nb")
ranef <- sim_list$ds$ranef
true_u_vec <- c(ranef[, 1:5])
u_name <- paste0()
true_u <- tibble(true = true_u_vec, variable = u_name)
true_u %>% tail()
