...
# simulate a data set
sim_data_set <- function(params, G, N, p_diff_exp = 0.2, p_up = 0.5, basefc = 4, 
                         sample_effects = TRUE, sig_u = 1) {
  idx <- sample(1:length(params$total$mean), size = G, replace = TRUE)
...
kparams <- readRDS("w_wout_test2/data/kparams.rds")
N_g <- 20
G <- 1e3
...
ds <- sim_data_set(params = kparams, G = G, N = N_g, basefc = 2)
...
saveRDS(sim_list, "w_wout_test2/data/sim_list.rds")