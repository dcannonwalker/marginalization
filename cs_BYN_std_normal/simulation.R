...
# simulate a data set
sim_data_set <- function(params, G, N, p_diff_exp = 0.2,  
                         sample_effects = TRUE) {
  idx <- sample(1:length(params$total$mean), size = G)
  mu1 <- params$total$mean[idx]
  mu2 <- mu1
  n_diff_exp <- round(G * p_diff_exp)
  fc <- exp(rnorm(n_diff_exp, sd = 3))
  mu2[1:n_diff_exp] <- mu2[1:n_diff_exp] * fc
...
ds <- sim_data_set(params = kparams, G = G, N = N_g)
...
saveRDS(sim_list, "cs_BYN_std_normal/sim_list.rds")
