...
stan_mu_b1 <- extract_variable_array(stan_post, "mu_b1")
plot(density(stan_mu_b1))
...
p_df <- stan_p %>%
  mutate(model = "stan", true = sim_list$D_g, b1 = sim_list$b1_g * sim_list$D_g)
...
b_df <- stan_b %>%
  mutate(model = "stan", 
         variable = stringr::str_sub(variable, 1, 2), 
         true_class = rep(sim_list$D_g, 2),
         est_class = rep(p_df$mean, 2),
         true = c(sim_list$b0_g, sim_list$b1_g * sim_list$D_g)) %>%
  mutate(mean = if_else(variable == "b1", mean * est_class, mean))
# ============== u ====================
stan_u <- stan_smry %>%
  ...
  select(variable, mean, sd) %>%
  mutate(model = "stan")
u_name <- paste0("u_g[", rep(1:250, each = 5), ",", rep(1:5, 250), "]")
...
gg2
saveRDS(gg1, "cs_eR2/data/gg1.rds")
saveRDS(gg2, "cs_eR2/data/gg2.rds")
...
# )
sig_b0 <- tibble(
  draw = seq(1, length(stan_sig_b0)),
  model = rep("stan", length(stan_sig_b0)),
  value = stan_sig_b0
)
true_sig_b0 <- 1
...
gg4
saveRDS(gg4, "cs_eR2/data/gg4.rds")

## sig_b1
sig_b1 <- tibble(
  draw = seq(1, length(stan_sig_b1)),
  model = rep("stan", length(stan_sig_b1)),
  value = stan_sig_b1)
...
gg5
saveRDS(gg5, "cs_eR2/data/gg5.rds")
...
gg6
saveRDS(gg6, "cs_eR2/data/gg6.rds")

## mu_b0
mu_b0 <- tibble(
  draw = seq(1, length(stan_mu_b0)),
  model = rep("stan", length(stan_mu_b0)),
  value = stan_mu_b0)
...
gg7
saveRDS(gg7, "cs_eR2/data/gg7.rds")

## mu_b11
mu_b1 <- tibble(
  draw = seq(1, length(stan_mu_b1)),
  model = rep("stan", length(stan_mu_b1)),
  value = stan_mu_b1)
gg8 <- ggplot(mu_b1, aes(value, fill = model)) + 
  geom_density(alpha = 0.6) + 
  geom_vline(xintercept = c(-2, 2), ) +
  annotate(x=c(-4, 4),y=+Inf,label="True mu_b1",vjust=2,geom="label") +
  theme_minimal() + 
  xlim(-10, 10) +
  ggtitle("Posterior density for mu_b1")  +
  xlab("value")

gg8
saveRDS(gg8, "cs_eR2/data/gg8.rds")
...
# 
# gg9
# saveRDS(gg9, "cs_eR2/data/gg9.rds")

## S
S <- tibble::as_tibble(stan_S) %>% 
